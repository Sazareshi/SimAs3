...
void CPly::routine_work(void *param) {
	...
	if (v_box_ref.x > pPubObj->pub_com.vfb_box.x) tmp_a.x = 1.0;
	else if(v_box_ref.x < pPubObj->pub_com.vfb_box.x) tmp_a.x = -1.0;
	else;

	if (v_box_ref.y > pPubObj->pub_com.vfb_box.y) tmp_a.y = 1.0;
	else if (v_box_ref.y < pPubObj->pub_com.vfb_box.y) tmp_a.y = -1.0;
	else;
...
LRESULT CALLBACK  CPly::PanelProc(HWND hDlg, UINT msg, WPARAM wp, LPARAM lp) {
	int n;
	...
	case WM_COMMAND:
		...
		case IDSET: {
			...
			/*
			//ƒTƒ“ƒvƒ‹‚Æ‚µ‚Ä‚¢‚ë‚¢‚ë‚ÈŒ^‚Å“Ç‚Ýž‚ñ‚Å•\Ž¦‚µ‚Ä‚¢‚é
			wstr += L"Param 1(d):";
...
void CPly::set_v_box_ref(WORD wp) {
	if (inf.panel_func_id == IDC_TASK_FUNC_RADIO1) {
		switch (wp) {
		case IDC_TASK_ITEM_RADIO1:
			v_box_ref.x = 0.0; break;
		case IDC_TASK_ITEM_RADIO2:
			v_box_ref.x = 2.0; break;
		case IDC_TASK_ITEM_RADIO3:
			v_box_ref.x = -2.0; break;
		case IDC_TASK_ITEM_RADIO4:
			v_box_ref.y = 0.0; break;
		case IDC_TASK_ITEM_RADIO5:
			v_box_ref.y = 2.0; break;
		case IDC_TASK_ITEM_RADIO6:
			v_box_ref.y = -2.0; break;
		default:
			v_box_ref.x = 0.0; break;
